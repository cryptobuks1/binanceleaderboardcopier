...
$query = "select coin_id,coin_digits, coin_abr, coin_logo, coin_fullname from coin_name where coin_abr <>'USDT'  and  coin_isvisible = '1' order by coin_id desc ";										
 ...
 if(! $link ) {
   ...
   while($row = mysqli_fetch_assoc($result)) {
				...
				extract($row);
				echo "<tr> <td>$coin_abr</td></td><td>$coin_fullname</td><td>$coin_digits</td><td style='text-align:center;'><img src='$coin_logo' style='max-width:25px;max-height:25px;' /><td style='text-align:center;'onclick='_edit(\"$coin_id\")'>Edit</td><td style='text-align:center;'onclick='_delete(\"$coin_id\",\"$coin_abr\")';'>Delete</td></tr>";
