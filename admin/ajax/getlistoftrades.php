...
print <<<here
 ...
 <ul class="list-group list-group-horizontal" style='color:b;font-weight:bolder;width:100%;'>
  ...
  <li class="list-group-item" style='font-size:small;width:100%;background:transparent;'>Estimated  PNL : <b style='zoom:1.21;' class='cumu_usd'>&nbsp;</b></li>
   <li class="list-group-item" style='font-size:small;width:100%;background:transparent;'>Draw Down : <b style='zoom:1.21;' class='cumu_dd'>&nbsp;</b></li>
...
$query = "select trade_by,trade_amount,trade_sl,trade_tp,trade_coin_digits,trade_id, trade_symbol, trade_leverage, trade_entryprice, trade_closedprice, DATE_FORMAT(FROM_UNIXTIME(trade_openedat), '%e %b %Y <br>(%H:%i:%s)')
...
(select coin_fullname from coin_name where coin_abr='USDT')tetherfullname , trade_estimatedloss , trade_estimatedprofit
from tbl_trades where trade_closedat IS NULL order by trade_openedat desc ";										
 ...
 if(! $link ) {
   ...
   while($row = mysqli_fetch_assoc($result)) {
				...
				echo "<tr class='$trade_hash' title= 'Estimated Profit: $trade_estimatedprofit USDT\nEstimated Loss: $trade_estimatedloss USDT\nFor SL and TP' > 
				...
				if($trade_by=="HANDNOTROBOT")
				{echo "<td  onclick='GLOBALPOSITIONHASH=\"$trade_hash\";  closeposition(this);'><i style = 'width:100%;margin:0auto;text-align:center;'class='fas fa-times'></i></a></td>";
				}
				else
				{
					echo "<td><sub style='zoom:0.8;'>Can't be closed manually</sub></td>";
				}
			}
