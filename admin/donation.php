			...
			if($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if($newpass == $newpassconf && (strlen($newpass)>4))
	{   mysqli_select_db($link, "binancefutures");
		$queryselectpass = "select admin_pass  as checkadminpass from tbl_admin where admin_id= '".$_SESSION['admin']."'";
		$resultpass = mysqli_query($link, $queryselectpass) or die(mysqli_error($link));
		  while($rowpass = mysqli_fetch_assoc($resultpass)) {
			  extract($rowpass);
		  }
		  
		  if($checkadminpass == $oldpass)
		  {
			  $queryupdate = "update tbl_admin set admin_pass = ".GetSQLValueString($newpass,"text")." where admin_id='".$_SESSION['admin']."'";
			  $resultupdate = mysqli_query($link, $queryupdate) or die(mysqli_error($link));
			  header("location:logout.php");
		  }
		  	else
	{
		  header("location:profileandsecurity.php?error=INVALID OLD PASS");
	}
	}
	else
	{
		  header("location:profileandsecurity.php?error=INVALID PASS");
	}
...
<body id="page-top">
<form action="profileandsecurity.php" method="post">
    <!-- Page Wrapper -->
    <div id="wrapper">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Content Row -->
                     <div class="container-fluid">
                        ...
                        <div class="card-body" style='zoom:0.96;'>
                       Any donation is accepted 🙁 <br>
					   It really taked a lot of time  to code this Bot . <br><hr>
					   You can also use my referral ID (If not already registred on binance) : <br>
					   <a href='https://accounts.binance.com/en/register?ref=39750507' target='_blank'>https://accounts.binance.com/en/register?ref=39750507</a>
					   <hr>
					   <b>ADA address:</b><br> DdzFFzCqrhsnce5Ufx3VEoh5pUrEFssxGc81LHDVLtNcj5CzDc39Z7dRkjugoQMP9NvYMVAQnWuUn7uzpwvmijVv8XBu9X9PSoYRbJWG<hr>
					   <b>BTC address:</b><br> 1KxiArHuQ2xT2eQRhwd3kKkfr4vY8oyuD<hr>
					   <b>DOT address:</b><br> 14oXb3bLRZEdqgbxvhiSuCb4Jjkc78FuX9yhj3s6pPw53aWZ<hr>
					   <b>ETH address:</b><br> 0xd35a8505c6b9750913ebef8f158baa83ab51df57<hr>
					   
					   
					   I really apperciate any donation 😘
